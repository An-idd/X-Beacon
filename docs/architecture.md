...
[Tracing]         ← 开启 trace span
   │
   ▼
[Auth]            ← API key 验证（Postgres + Redis 缓存 + singleflight）
   │
   ▼
[RateLimit]       ← 检查限流（memory bucket + Redis sliding window）
   │
   ▼
[Smart Route]     ← Week 11: 规则引擎改写 req.Model（A/B opt-out via scope）
   │
   ▼
...
[Cache Read]      ← 精确（sha256） → 语义（HNSW KNN + threshold）
   ...
   │                                     │
   ▼ Miss                                │
[Router]          ← Week 6: retry + chain failover + circuit breaker
   │                                     │
   ▼                                     │
...
- Smart Route 在 cache 之前：路由后的 model 进 cache key（A/B 控制组用 scope 取消路由后会拿到独立 cache slot，避免对照组污染）
- Prompt Compress 在 Smart Route 之后、Cache 之前：路由决策看完整意图；cache key 反映真实送给上游的形态
